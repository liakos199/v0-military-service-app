...
export function toast(message: string, type: 'success' | 'error' | 'info' = 'success') {
  if (type === 'success') {
    sonnerToast.success(message, {
      className: 'bg-zinc-900 border-emerald-500/50 text-white font-bold text-[11px] uppercase tracking-wider',
    })
  } else if (type === 'error') {
    sonnerToast.error(message, {
      className: 'bg-zinc-900 border-red-500/50 text-white font-bold text-[11px] uppercase tracking-wider',
    })
  } else {
    sonnerToast(message, {
      className: 'bg-zinc-900 border-zinc-700 text-white font-bold text-[11px] uppercase tracking-wider',
    })
  }
}
...
export function exportAppData() {
  ...
  link.href = url
  link.download = `apolele-pro-backup-${date}.json`
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
  URL.revokeObjectURL(url)
...
export async function importAppData(file: File): Promise<boolean> {
