...
export function OnboardingFlow({ isOpen, onComplete }: { isOpen: boolean; onComplete: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-background flex flex-col overflow-hidden safe-top" style={{ background: 'linear-gradient(180deg, #050705 0%, #0D110D 100%)' }}>
      {/* Progress Bar */}
      <div className="absolute top-0 left-0 w-full h-1 bg-zinc-900 z-10">
        <div 
      ...
      {/* Header */}
      <div className="flex-shrink-0 px-4 py-4 border-b border-border/50 mt-1">
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Βήμα {step} από {totalSteps}</p>
        </div>
      </div>
          ...
          {step === 3 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-1.5">
                ...
                <select 
                  className="w-full bg-zinc-900 border-zinc-800 h-12 rounded-xl px-3 text-sm focus:ring-primary/50 outline-none border text-foreground"
                  value={formData.rank}
              ...
              <div className="bg-primary/5 border border-primary/10 rounded-xl p-3 flex items-start gap-3 mt-4">
      ...
      <div className="flex-shrink-0 px-4 py-4 border-t border-border/50 bg-background/50 backdrop-blur-sm">
        <div className="flex gap-3">
          ...
          <Button 
            onClick={handleNext}
            className={cn(
              "bg-primary hover:bg-primary/90 text-primary-foreground font-bold h-12 rounded-xl shadow-lg shadow-primary/20",
